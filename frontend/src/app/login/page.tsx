...
import { seConnecter, decoderToken, redirectionSelonRole, recupererMonProfil } from "../../lib/auth";
...
export default function LoginPage() {
  ...
  async function onSubmit(data: LoginFormValues) {
    ...
    try {
      // Connexion commune : Patient, Médecin, Hôpital, Super Admin
      const reponse = await seConnecter(data.email, data.motDePasse);

      // Lecture du JWT : { sub, email, role }
      const payload = decoderToken(reponse.accessToken);

      if (payload.role === "PATIENT") {
        // Profil incomplet (pas de téléphone) → on l'envoie compléter son profil
        const profil = await recupererMonProfil();
        router.push(profil.telephone ? "/patient/dashboard" : "/patient/monprofil?bienvenue=1");
      } else {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-canvas p-8">
      <div className="grid w-full max-w-[980px] animate-cardIn grid-cols-1 overflow-hidden rounded-[28px] bg-white shadow-[0_30px_70px_-20px_rgba(15,42,32,0.28)] md:grid-cols-[44%_56%]">
        <AuthGreetPanel
          title="Bonjour"
          subtitle="Connectez-vous à MadaCare pour accéder à votre espace personnel ou établissement."
        />
        ...
        <main className="flex flex-col justify-center p-9 md:p-12">
          ...
          <p className="mt-3 text-center text-[13px] text-mist">
            ...
            <Link href="/hopital/register" className="font-semibold text-emerald-600 hover:underline">
              Inscrire un hôpital
            </Link>
          </p>
        </main>
